...
def is_model_encoder_only(model = None):
    ...
    if model in [ModelBackboneTypes.LLAMA.value, ModelBackboneTypes.MISTRAL.value,
                 ModelBackboneTypes.CAUSAL_LM.value]:
        return False
...
def get_model_special_tokens(model, tokenizer):
    if is_model_encoder_only(model):
        ...
        if not any(item is not None for item in token_ids):
            token_ids = getattr(tokenizer, "all_special_ids", [])

    return list(dict.fromkeys(item for item in token_ids if item is not None))


def is_use_prompt():
    if ExpArgs.explained_model_backbone == ModelBackboneTypes.CAUSAL_LM.value:
        return True
    return (
        (not is_model_encoder_only(ExpArgs.explained_model_backbone))
        and (not ExpArgs.task.is_llm_use_lora)
        and (not getattr(ExpArgs, "llm_adapter_path", None))
    )
...
def run_model(model, model_backbone, input_ids: Tensor = None, attention_mask: Tensor = None,
    ...
    if is_model_encoder_only(model_backbone):
        ...
        if is_use_prompt():
            logits = logits[:, -1, :][:, ExpArgs.label_vocab_tokens.to(logits.device)]
...
def merge_prompts(inputs, attention_mask, task_prompt: Tensor = None, label_prompt: Tensor = None,
    ...
    return merged_inputs.to(get_device()), merged_attention_mask.to(get_device())
